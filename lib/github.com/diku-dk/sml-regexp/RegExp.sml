...
struct
    ...
    val insert      = IntSet.insert
    val fold        = IntSet.fold
    fun mem s i     = IntSet.member(i,s)
...
struct
    type 'a map = 'a IntMap.map
    exception NotFound
    fun insert m i v = IntMap.add(i,v,m)
    val empty = fn () => IntMap.empty
    fun lookup m i = case IntMap.lookup m i of
                         SOME x => x
                       | NONE => raise NotFound
    val fold = IntMap.fold
end
